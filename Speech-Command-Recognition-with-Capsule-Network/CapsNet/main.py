    ...
    if args.is_training == 'TRAIN':
        ...
        if args.keep == 0:
            ...
            for i in range(6):
                ...
                if args.test_by=='noise':teX, teY = load_specific_noisy_data(args.data_path, 'TEST', args.mode, args.feature_len, noise_list[i], open_set=args.open_set);cprint(noise_list[i],'red')
                elif args.test_by=='echo':teX, teY = load_specific_noisy_data(args.data_path, 'TEST', args.mode, args.feature_len, 'echo', open_set=args.open_set);cprint('echo','red')
                else: teX, teY = load_random_noisy_data(args.data_path,'TEST',args.mode, args.feature_len, SNR=args.SNR, open_set=args.open_set)
